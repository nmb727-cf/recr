...
class WorkflowSLAEngine:
    ...
    def create_sla_execution(tenant_id, policy: WorkflowSLAPolicy, entity_type, entity_id, workflow_id=None, owner_id=None, metadata=None):
        ...
        existing = WorkflowSLAExecution.objects.filter(
            tenant_id=tenant_id,
            sla_policy=policy,
            entity_type=entity_type,
            entity_id=str(entity_id),
            current_status__in=[
                SLAExecutionStatus.ACTIVE,
                SLAExecutionStatus.WARNING_DUE,
                SLAExecutionStatus.BREACHED,
                SLAExecutionStatus.ESCALATED,
            ],
    ...
    def schedule_warning_reminder(execution: WorkflowSLAExecution):
        if not execution.warning_at or execution.warning_at < timezone.now():
            return
        if WorkflowSLAReminder.objects.filter(
            sla_execution=execution,
            reminder_type=SLAReminderType.WARNING,
            status__in=[
                SLAReminderStatus.SCHEDULED,
                SLAReminderStatus.SENT,
            ],
        ).exists():
            return
    ...
    def process_escalation(execution: WorkflowSLAExecution):
        ...
        if rule:
            if WorkflowSLAReminder.objects.filter(
                ...
                metadata__escalation_level=next_level,
            ).exists():
                return False
    ...
    def cancel_sla(tenant_id, entity_type, entity_id):
        ...
        if execution_ids:
            WorkflowSLAReminder.objects.filter(
                sla_execution_id__in=execution_ids,
                status=SLAReminderStatus.SCHEDULED,
            ).update(status=SLAReminderStatus.CANCELLED, updated_at=timezone.now())
        return count
