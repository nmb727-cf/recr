...
class CandidateEngagement(models.Model):
    ...
    priority = models.CharField(max_length=10, choices=PRIORITY_CHOICES, default='warm')
    intent = models.CharField(max_length=50, choices=INTENT_CHOICES, default='just_lead', db_index=True)
    
    # Next Action System
    next_action_type = models.CharField(max_length=50, choices=NEXT_ACTION_CHOICES, null=True, blank=True)
    next_action_due_at = models.DateTimeField(null=True, blank=True, db_index=True)
    next_action_owner_id = models.UUIDField(null=True, blank=True)
    next_action_priority = models.CharField(max_length=20, choices=PRIORITY_CHOICES, default='warm')
    next_action_reminder_minutes = models.IntegerField(null=True, blank=True)
    
    is_nurture_track = models.BooleanField(default=False, db_index=True)
