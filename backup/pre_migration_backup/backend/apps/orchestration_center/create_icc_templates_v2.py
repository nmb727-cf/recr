...
def create_templates():
    ...
    for name in prequal_names:
        form, _ = PrequalForm.objects.get_or_create(
            tenant_id=tenant_id,
            name=name,
            defaults={
        ...
        section, _ = PrequalSection.objects.get_or_create(
            tenant_id=tenant_id,
            form=form,
            title="General Questions",
            defaults={'order': 1}
        )
        PrequalQuestion.objects.get_or_create(
            tenant_id=tenant_id,
            section=section,
    ...
    for name in scorecard_names:
        scorecard, _ = InterviewScorecardTemplate.objects.get_or_create(
            tenant_id=tenant_id,
            name=name,
            defaults={
        ...
        InterviewScorecardAttribute.objects.get_or_create(
            tenant_id=tenant_id,
            scorecard=scorecard,
